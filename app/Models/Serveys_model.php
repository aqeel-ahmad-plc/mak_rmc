...
class Serveys_model extends Model
{
    ...
    public function getServeys($servey_id = null)
    {
        ...
        if (!$servey_id)
        {
             $query = $this->builder()->select('sites.site_id as siteid,sites.status as is_surveyed,sites.is_installed,sites.fat_status,sites.handing_taking_status, sites.masgid,sites.district,sites.tehsil,sites.na_pk,sites.package,sites.uc_vc_name_and_no, servey.*')->join('sites', 'sites.id = servey.site_id')->get();
    ...
    public function getServeysReport($servey_filter = null, $sub_filter = null)
    {
        ...
        if($sub_filter == "package"){
            $query = $this->builder()->select('sites.site_id as siteid,sites.status as is_surveyed,sites.is_installed,sites.fat_status,sites.handing_taking_status, sites.masgid,sites.district,sites.tehsil,sites.na_pk,sites.package,sites.uc_vc_name_and_no, servey.*')->join('sites', 'sites.id = servey.site_id')->where(['sites.package' => $servey_filter])->get();
        }

        elseif($sub_filter == "district"){
            $query = $this->builder()->select('sites.site_id as siteid,sites.status as is_surveyed,sites.is_installed,sites.fat_status,sites.handing_taking_status, sites.masgid,sites.district,sites.tehsil,sites.na_pk,sites.package,sites.uc_vc_name_and_no, servey.*')->join('sites', 'sites.id = servey.site_id')->where(['sites.district' => $servey_filter])->get();
        }
