...
use App\Models\Problematic_model;
...
class Problematic extends BaseController
{
	public function show()
	{
		$problematic_model  = new Problematic_model();
        $data['serveys']  = $problematic_model->getServeys();

        echo view('templates/header', $data);
        echo view('templates/sidebar');
		echo view('problematic/show');
	...
	public function print_status_and_feasibility($id)
	{
		$data 				= [];
		$problematic_model  	= new Problematic_model();
		$serveys			= $problematic_model->getServeys();
	...
	{
		$problematic_model  = new Problematic_model();
        $data['servey']  = $problematic_model->getServeys($id);

        echo view('templates/header', $data);
        echo view('templates/sidebar');
		echo view('problematic/view_survey');
		echo view('templates/footer');
	}
    ...
    public function manage()
	{
		...
		{
			$servey = [];
			$servey  = $problematic_model->getServeys($sites[$i]['id']);
			if($servey)
			{
    ...
    public function create($site_id)
	{
		...
		if ($this->request->getMethod() == 'post') {
			$rules = [
                ...
                'gps_coordinates_n'             => [
						'label' => 'GPS Coordinates (N)',
						'rules' => 'required|max_length[255]'
				],
                'gps_coordinates_e'             => [
						'label' => 'GPS Coordinates (E)',
						'rules' => 'required|max_length[255]'
				],
			...
			if (! $this->validate($rules)) {
				...
				$servey_model = new Problematic_model();
                ...
                $newData = [
                    ...
                    'gps_coordinates_n'             => $this->request->getVar('gps_coordinates_n'),
                    'gps_coordinates_e'             => $this->request->getVar('gps_coordinates_e'),
	...
	public function survey_images($site_id)
	{
		$data 			= [];
		$problematic_model  = new Problematic_model();
        $data['servey'] = $problematic_model->getServeys($site_id);
		// dd($data);
		if ($this->request->getMethod() == 'post') {
	...
	public function approve($site_id)
	{
		...
		$problematic_model->updateServeyStatus($site_id, $servey_status);
		$session = session();
